...
pid_t zygote_find_process(void)
{
	...
	while ((dp = readdir(dir)) != NULL) {
		...
		if (len <= 0) {
			// LOGD("process info is null:%s\n", buf2);
			continue;
		}

		buf[len] = 0;
		if (fnmatch(buf, "/system/bin/app_process", FNM_PATHNAME) != 0) {
			// LOGD("process info is mismatch:%s\n", buf);
			continue;
		}
...
pid_t ptrace_zygote(pid_t zygote_pid)
{
	...
	if (ptrace_attach(zygote_pid)) {
		LOGD("Error: ptrace zygote failed\n");
		return -1;
	}

	/* use this option to automatically attach to the child forked from zygote */
	...
	while (1) {
		...
		if (pid == -1) {
			...
			LOGD("Error: ptrace zygote waitpid failed\n");
			return -1;
		}
		/* assert(zygote_pid == pid); */
		LOGD("zygote event: pid=%d, status=%x\n", pid, status);
		/* check if the fork() event happens */
		if (IS_FORK_EVENT(status)) {
			pid_t newpid;
			/* get the child pid from the msg */
